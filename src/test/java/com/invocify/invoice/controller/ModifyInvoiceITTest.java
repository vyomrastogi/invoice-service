...
public class ModifyInvoiceITTest {
  ...
  public void addLineItemsToNonExistingInvoice() throws Exception {
    ...
    LineItem lineItem4 =
        LineItem.builder()
            .description("rate based line item4")
            .quantity(3)
            .rate(new BigDecimal(5.7))
            .rateType("rate")
            .build();
    ...
    mockMvc
        .perform(
            patch("/api/v1/invocify/invoices/{invoiceId}/lineItems", randomUUID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(patchLineItems)))
        .andExpect(status().isNotFound())
        .andExpect(
            jsonPath("$.errors.[0]")
                .value(String.format("Given Invoice not found: %s", randomUUID.toString())));
  }
  ...
  public void addLineItemsToExistingUnpaidInvoice() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/v1/invocify/invoices/{invoiceId}/lineItems", invoice1.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(patchLineItems)))
  ...
  public void addLineItemsToExistingPaidInvoice() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/v1/invocify/invoices/{invoiceId}/lineItems", invoice.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(patchLineItems)))
        ...
        .andExpect(
            jsonPath("$.errors[0]")
                .value(
                    String.format(
                        "Paid Invoice cannot be updated: %s", invoice.getId().toString())));
  }
  ...
  public void updateExistingUnpaidInvoice() throws Exception {
    ...
    LineItem lineItem2 =
        LineItem.builder()
            .description("Service line item 2")
            .quantity(1)
            .rate(new BigDecimal(150.3))
            .rateType("flat")
            .build();
    ...
    InvoiceUpdateRequest invoiceUpdateRequest =
        new InvoiceUpdateRequest("tech person", company.getId(), lineItemList, true);

    mockMvc
        .perform(
            put("/api/v1/invocify/invoices/" + invoice.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(invoiceUpdateRequest)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data.id").value(invoice.getId().toString()))
        .andExpect(jsonPath("$.data.author").value(invoiceUpdateRequest.getAuthor()))
  ...
  public void updatePaidInvoice() throws Exception {
    ...
    InvoiceUpdateRequest invoiceUpdateRequest =
        new InvoiceUpdateRequest("tech person", company.getId(), lineItemList, false);
  ...
  public void updateNonExistingInvoice() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/v1/invocify/invoices/" + invoiceRandomUUID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(invoiceUpdateRequest)))
        .andExpect(status().isNotFound())
        .andExpect(
            jsonPath("$.errors.[0]")
                .value(String.format("Given Invoice not found: %s", invoiceRandomUUID)));
  }
  ...
  public void updateExistingInvoiceWithUnavailableCompany() throws Exception {
    ...
    InvoiceUpdateRequest invoiceUpdateRequest =
        new InvoiceUpdateRequest("tech person", companyRandomUUID, lineItemList, false);
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.errors.[0]")
                .value(String.format("Given company not found: %s", companyRandomUUID.toString())));
  }
  ...
  public void updateExistingInvoiceWithInvalidRequest() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.errors.[2]")
                .value("Invoice should be associated with an existing company"));
  }
