...
public class InvoiceService extends InvocifyServiceHelper {
  ...
  /** repository is getting injected using constructor provided by lombok.AllArgsConstructor */
  ...
  public Invoice addLineItemsToInvoice(UUID invoiceId, List<LineItem> lineItems)
  ...
  public Page<Invoice> getInvoices(
      int page, long chronoValue, ChronoUnit chronoUnit, boolean disableFilter) {
    Pageable sortByDateWithTenEntries =
        PageRequest.of(page, NUMBER_OF_ELEMENTS, Sort.by("lastModifiedDate").descending());
    return disableFilter
        ? invoiceRepository.findAll(sortByDateWithTenEntries)
        : invoiceRepository.findByLastModifiedDateAfter(
            sortByDateWithTenEntries, filterDate(chronoValue, chronoUnit));
  }
  ...
  public Invoice updateInvoice(UUID invoiceId, InvoiceUpdateRequest invoiceRequest)
  ...
  private Invoice buildUpdatedInvoiceEntity(
      Invoice invoice, InvoiceUpdateRequest invoiceRequest, Company company) {
    return Invoice.builder()
        .id(invoice.getId())
        .author(invoiceRequest.getAuthor())
        .company(company)
        .lineItems(invoiceRequest.getLineItems())
        .paidStatus(invoiceRequest.isPaidStatus())
        .build();
  }
}
